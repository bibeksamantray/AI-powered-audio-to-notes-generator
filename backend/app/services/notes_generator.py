...
def build_notes_prompt(
    ...
    """
    Build a clear prompt for the Groq model to convert a lecture transcript
...
- Do NOT explain that you are summarizing or analyzing — just produce notes
...
def generate_notes_with_groq(
    ...
    """
    Call Groq to generate lecture notes from a transcript.
    """
    ...
    logger.info(
        "Requesting notes from Groq model '%s'...",
    ...
    try:
        ...
        response = client.chat.completions.create(
            ...
            messages=[
                {
                    "role": "system",
                    "content": "You are a highly accurate academic teaching assistant.",
                },
                {
                    "role": "user",
                    "content": prompt,
        ...
        if response.choices and response.choices[0].message:
            notes_text = (response.choices[0].message.content or "").strip()

        if not notes_text:
            raise RuntimeError("Received empty notes from Groq.")

        logger.info("Notes successfully generated by Groq.")
        return notes_text
    except Exception as exc:
        logger.exception("Failed to generate notes via Groq: %s", exc)
        raise
